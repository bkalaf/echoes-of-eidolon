...
if [[ ! -d "$EIDOLON_ATLAS_RELEASE_ROOT" || -L "$EIDOLON_ATLAS_RELEASE_ROOT" \
  || ! -f "$EIDOLON_ATLAS_RELEASE_ROOT/DEPLOYMENT_DATASET_MANIFEST.json" \
  || ! -f "$EIDOLON_ATLAS_RELEASE_ROOT/FILE_MANIFEST.json" \
  || ! -f "$EIDOLON_ATLAS_RELEASE_ROOT/R09_AUTHORITATIVE_RELEASE_MANIFEST.json" ]]; then
  echo "EIDOLON_ATLAS_RELEASE_ROOT must be the extracted non-symlink R09 deployment dataset." >&2
  exit 2
fi
...
rollback_application() {
  ...
  run_unlocked test -s "$EIDOLON_REPOSITORY_DIR/apps/web/.output/server/index.mjs" || return
