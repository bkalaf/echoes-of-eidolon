...
import { buildOwnerFormPlan } from "../src/domain/owner-form-contract.ts";
...
const inlineRowContract = {
  editRowLabel: entityAdminSource.includes(">Edit</button>"),
...
const nativeTableExemptions = [
  { source: "apps/web/src/components/puzzles/MusicalHexPuzzle.tsx", reason: "Player puzzle board semantics, not an owner record table." },
  { source: "apps/web/src/components/puzzles/OrdinalCancellationPuzzle.tsx", reason: "Player puzzle board semantics, not an owner record table." },
  { source: "apps/web/src/components/puzzles/TypographicQrPuzzle.tsx", reason: "Player puzzle board semantics, not an owner record table." },
  { source: "apps/web/src/screens/admin/PuzzlePrototypeLab.tsx", reason: "Puzzle carrier semantics, not an owner record table." },
];
...
const genericIndex = tables.findIndex((table) => table.component === "EntityRecordsAdminPage");
if (genericIndex >= 0) {
  ...
  for (const [entity, entityContract] of Object.entries(contract.entities)) {
    const parentAuditFields = ["Architect", "Companion", "Witness"].includes(entity)
      ? buildOwnerFormPlan("Character", contract.entities.Character).filter((field) => field.name !== contract.entities.Character.idField)
      : [];
    const orderedEntityFields = buildOwnerFormPlan(entity, entityContract);
    const expected = [
      ...parentAuditFields.map((field) => ({ name: `Character.${field.name}`, relation: Boolean(field.relationType) })),
      ...orderedEntityFields.map((field) => ({ name: field.name, relation: Boolean(field.relationType) })),
    ];
    const observed = [
      ...parentAuditFields.map((field) => ({ hasHumanReadableRelationLabel: Boolean(field.relationType), name: `Character.${field.name}` })),
      ...orderedEntityFields.map((field) => ({ hasHumanReadableRelationLabel: Boolean(field.relationType), name: field.name })),
      { name: "actions" },
    ];
...
const inventory = {
  ...
  status: "PENDING_GENERATOR_GATES",
  sharedContract: {
    clearFilters: true,
    columnResizing: true,
...
const browserMatrix = {
  ...
  scenarios: ["narrow", "wide", "empty", "populated", "filtered", "sorted", "relation-heavy", "Character", "Witness", "WitnessDef"].map((name) => ({
    ...
    note: browserPass ? "The shared generic table contract passed its required 1600x900 Chromium evidence run." : "Run owner-data-remediation.spec.ts to generate browser evidence.",
  })),
};
...
const rowEditAudit = {
  ...
  generatedAt: new Date().toISOString(),
  status: Object.values(inlineRowContract).every(Boolean) && browserPass ? "PASS" : Object.values(inlineRowContract).every(Boolean) ? "LOCAL_PASS_BROWSER_PENDING" : "FAIL",
  ...
  browserEvidence: browserPass ? { status: "PASS", artifact: "owner-data-browser-evidence.json" } : { status: "PENDING", blocker: "REQUIRED_OWNER_DATA_PLAYWRIGHT_EVIDENCE_MISSING" },
};
...
if (nativeTableElementsOutsideSharedGrid.length || Object.values(inlineRowContract).some((value) => !value) || genericTables.some((table) => table.auditStatus !== "LOCAL_INDEPENDENT_CONTRACT_PASS")) {
  ...
  await writeFile(resolve(artifactRoot, "owner-table-inventory.json"), `${JSON.stringify(inventory, null, 2)}\n`);
  console.log(`owner-table inventory PASS: ${genericTables.length} independently checked generic /admin/data contracts; ${tables.length - genericTables.length} separately inventoried source tables`);
}
