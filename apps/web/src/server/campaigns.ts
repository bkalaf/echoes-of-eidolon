...
import { AbilityType, ArchitectDepartment, AwarenessSkill, CompanionKey, EntityType, Faction, Heirloom, Holiday, InterludeType, KnowledgeSkill, TimelineEventType, WitnessColor, WorldKey } from "../generated/prisma/enums";
...
export async function createCampaignCatalogItem(input: z.infer<typeof campaignCatalogCreateSchema>, database: Database = getDatabase()) {
  ...
  return database.$transaction(async (transaction) => {
    ...
    if (objectType === "ARCHITECT") { const row = z.object({ architectId: identity, character: characterCreateSchema, department: z.enum(ArchitectDepartment) }).strict().parse(payload); return transaction.character.create({ data: { ...row.character, architect: { create: { architectId: row.architectId, department: row.department } } }, include: { architect: true } }); }
    if (objectType === "WITNESS") { const row = z.object({ witnessId: identity, character: characterCreateSchema, witnessDef: z.object({ witnessDefId: identity, name: z.string().trim().min(1), department: z.enum(ArchitectDepartment), apparentDomain: z.string().trim().min(1), realDomain: z.string().trim().min(1), color: z.enum(WitnessColor) }).strict(), trueFlawName: z.string().trim().min(1), architectId: identity, legendaryRewardId: identity, constellationBeforeId: identity.nullable().optional(), constellationAfterId: identity.nullable().optional() }).strict().parse(payload); await transaction.witnessDef.create({ data: row.witnessDef }); return transaction.character.create({ data: { ...row.character, witness: { create: { witnessId: row.witnessId, witnessDefId: row.witnessDef.witnessDefId, trueFlawName: row.trueFlawName, architectId: row.architectId, legendaryRewardId: row.legendaryRewardId, constellationBeforeId: row.constellationBeforeId ?? null, constellationAfterId: row.constellationAfterId ?? null } } }, include: { witness: true } }); }
    if (objectType === "COMPANION") { const row = z.object({ companionKey: z.enum(CompanionKey), soulId: identity, heirloom: z.enum(Heirloom), knowledgeSkill: z.enum(KnowledgeSkill), awarenessSkill: z.enum(AwarenessSkill), concordCharacter: characterCreateSchema, ruinCharacter: characterCreateSchema, schismCharacter: characterCreateSchema }).strict().parse(payload); const slots = [["CONCORD", row.concordCharacter], ["RUIN", row.ruinCharacter], ["SCHISM", row.schismCharacter]] as const; for (const [worldKey, character] of slots) { if (character.worldKey !== worldKey || character.soulId !== row.soulId) throw new CampaignBookRangeError(`${worldKey} Companion Character must use ${worldKey} and Soul ${row.soulId}.`); await transaction.character.create({ data: character }); } await transaction.companionDef.create({ data: { companionKey: row.companionKey, soulId: row.soulId, heirloom: row.heirloom, knowledgeSkill: row.knowledgeSkill, awarenessSkill: row.awarenessSkill, concordCharacterId: row.concordCharacter.characterId, ruinCharacterId: row.ruinCharacter.characterId, schismCharacterId: row.schismCharacter.characterId } }); await transaction.companion.createMany({ data: slots.map(([, character]) => ({ characterId: character.characterId, companionKey: row.companionKey })) }); return { companionKey: row.companionKey }; }
...
async function campaignCatalog(database: Database): Promise<Record<CampaignObjectType, CampaignCatalogItem[]>> {
  const [pillars, lessons, timelineEvents, transitions, interludes, companions, witnesses, architects, rewards] = await Promise.all([
    ...
    database.witness.findMany({ select: { witnessId: true, character: { select: { displayName: true } } }, orderBy: { witnessId: "asc" } }),
    database.architect.findMany({ select: { architectId: true, character: { select: { displayName: true } } }, orderBy: { architectId: "asc" } }),
  ...
  for (const item of witnesses) add("WITNESS", item.witnessId, item.character.displayName);
  for (const item of architects) add("ARCHITECT", item.architectId, item.character.displayName);
