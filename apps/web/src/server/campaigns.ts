...
export async function createCampaignCatalogItem(input: z.infer<typeof campaignCatalogCreateSchema>, database: Database = getDatabase()) {
  ...
  return database.$transaction(async (transaction) => {
    ...
    if (objectType === "ARCHITECT") { const row = z.object({ character: characterCreateSchema, department: z.enum(ArchitectDepartment) }).strict().parse(payload); return transaction.character.create({ data: { ...row.character, architect: { create: { department: row.department } } }, include: { architect: true } }); }
    if (objectType === "WITNESS") { const row = z.object({ character: characterCreateSchema, witnessDef: z.object({ witnessDefId: identity, name: z.string().trim().min(1), department: z.enum(ArchitectDepartment), apparentDomain: z.string().trim().min(1), realDomain: z.string().trim().min(1), color: z.enum(WitnessColor) }).strict(), trueFlawName: z.string().trim().min(1), architectCharacterId: identity, legendaryRewardId: identity, constellationBeforeId: identity.nullable().optional(), constellationAfterId: identity.nullable().optional() }).strict().parse(payload); await transaction.witnessDef.create({ data: row.witnessDef }); return transaction.character.create({ data: { ...row.character, witness: { create: { witnessDefId: row.witnessDef.witnessDefId, trueFlawName: row.trueFlawName, architectCharacterId: row.architectCharacterId, legendaryRewardId: row.legendaryRewardId, constellationBeforeId: row.constellationBeforeId ?? null, constellationAfterId: row.constellationAfterId ?? null } } }, include: { witness: true } }); }
