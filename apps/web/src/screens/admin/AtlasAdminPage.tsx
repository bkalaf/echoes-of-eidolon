...
function PoiAtlas({ atlas, globe }: { atlas: AtlasCatalogProjection; globe: boolean }) {
  ...
  const globeLocations = useMemo(() => atlas.pointsOfInterest.map((point) => ({ id: point.poiId, label: point.displayName ?? point.workingLabel, latticeId: point.latticeId, latitude: point.latitude, longitude: point.longitude, regionId: point.regionId })), [atlas.pointsOfInterest]);
  return <><div className="tabs"><a className={!globe ? "active" : ""} href="/admin/atlas/pois?state=ATLAS_POI_2D">2D Map</a><a className={globe ? "active" : ""} href="/admin/atlas/pois?state=ATLAS_POI_3D">3D Globe</a></div><div className="atlas-layout"><section>{globe ? <AtlasGlobe locations={globeLocations} onSelect={setSelectedId} selectedId={selectedId} /> : <AtlasMapViewport imageAlt="Eidolon world map" imageSrc={managedAssetUrl("atlas.official-world-founding-cities")} onSelect={setSelectedId} points={atlas.pointsOfInterest.map((point) => ({ id: point.poiId, label: `Select ${point.displayName ?? point.workingLabel}`, latitude: point.latitude, longitude: point.longitude }))} selectedId={selectedId} />}</section><PoiDetail atlas={atlas} selectedId={selectedId} /></div><section className="card"><DataTable columns={poiColumns} data={atlas.pointsOfInterest} getRowId={(point) => point.poiId} onRowActivate={(point) => setSelectedId(point.poiId)} preferenceKey="admin.atlas.points-of-interest" rowClassName={(point) => point.poiId === selectedId ? "selected-row" : undefined} /></section><p className="notice">{atlas.pointsOfInterest.length} canonical Points of Interest · {atlas.coordinateReferenceSystem} · {atlas.releaseId}</p></>;
}
