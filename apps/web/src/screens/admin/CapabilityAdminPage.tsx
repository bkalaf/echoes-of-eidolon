...
function ScoringPolicies() {
  const resource = useJsonResource<{ rewardPolicies: Array<{ rewardScoringPolicyId: string; version: number; status: string; minimumScore: number; maximumScore: number; weights: Array<{ kind: string; weight: number }> }>; factionPolicies: unknown[]; candidates: unknown[] }>("/api/admin/capabilities/scoring");
  const active = resource.data?.rewardPolicies.find((policy) => policy.status === "ACTIVE");
  const [message, setMessage] = useState("");
  const createDraft = async () => { if (!active) return; const response = await fetch("/api/admin/capabilities/scoring", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ minimumScore: active.minimumScore, maximumScore: active.maximumScore, weights: Object.fromEntries(active.weights.map(({ kind, weight }) => [kind, weight])) }) }); setMessage(response.ok ? "Draft policy version created." : "Draft creation failed."); resource.reload(); };
  const activate = async (policyId: string) => { const response = await fetch(`/api/admin/capabilities/scoring/${encodeURIComponent(policyId)}/activate`, { method: "POST" }); setMessage(response.ok ? "Reward scoring policy activated." : "Policy activation failed."); resource.reload(); };
  return <section className="card"><div className="action-row action-row--between"><div><h2>Evidence Scoring Policies</h2><p>Semantic evidence retains the policy version that interpreted it.</p></div><button className="button button--gold" disabled={!active} onClick={() => void createDraft()}>Create Draft Copy</button></div>{resource.loading ? <p>Loading…</p> : resource.error ? <p className="notice notice--warn">{resource.error}</p> : <>{resource.data?.rewardPolicies.map((policy) => <article className="card" key={policy.rewardScoringPolicyId}><div className="action-row action-row--between"><h3>Reward policy v{policy.version} · {policy.status}</h3>{policy.status === "DRAFT" && <button className="button" type="button" onClick={() => void activate(policy.rewardScoringPolicyId)}>Activate</button>}</div><p>Range {policy.minimumScore}–{policy.maximumScore}</p><table className="simple-table"><tbody>{policy.weights.map((weight) => <tr key={weight.kind}><td>{weight.kind}</td><td>{weight.weight > 0 ? "+" : ""}{weight.weight}</td></tr>)}</tbody></table></article>)}<p className="notice notice--warn">Faction standing has no owner-authoritative weights. Runtime remains fail-closed.</p></>}{message && <p>{message}</p>}</section>;
}
