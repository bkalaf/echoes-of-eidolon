...
export function OccupationAdminPage() {
  ...
  const editOccupation = (occupation: OccupationRecord) => { setKey(occupation.occupationId); setName(occupation.name); setDescription(occupation.description ?? ""); setActive(occupation.active); setAffinity(occupation.affinities.map((entry) => entry.abilityType)); setEditing(true); setMessage(""); };
  const occupationColumns: DataTableColumnDef<OccupationRecord>[] = [
    { accessorKey: "name", header: "Occupation" },
    { accessorKey: "occupationId", header: "Occupation ID" },
    { accessorFn: (occupation) => occupation.active ? "Yes" : "No", header: "Active", id: "active" },
    { accessorKey: "description", header: "Description" },
    { accessorFn: (occupation) => [...occupation.affinities].sort((left, right) => left.ordinal - right.ordinal).map((entry) => `${entry.ordinal}:${entry.abilityType}`).join(" → "), header: "Ordered affinity", id: "affinities" },
    { cell: ({ row }) => <button className="button" onClick={() => editOccupation(row.original)} type="button">Edit {row.original.name}</button>, enableColumnFilter: false, enableSorting: false, header: "Actions", id: "actions" },
  ];
  return <div className="split"><section className="card"><div className="action-row action-row--between"><h2>Occupations</h2><button className="button" onClick={reset} type="button">New Occupation</button></div>{query.isPending ? <p>Loading…</p> : query.isError ? <p className="notice notice--bad">{query.error.message}</p> : <DataTable columns={occupationColumns} data={query.data} getRowId={(occupation) => occupation.occupationId} preferenceKey="admin.occupations" />}</section><section className="card"><h2>Occupation attributes</h2><label className="field">Stable key<input className="input" onChange={(event) => setKey(event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, "_"))} readOnly={editing} value={key} /></label><label className="field">Display name<input className="input" onChange={(event) => setName(event.target.value)} value={name} /></label><label className="field">Description<textarea className="textarea" onChange={(event) => setDescription(event.target.value)} value={description} /></label><label className="field"><input checked={active} onChange={(event) => setActive(event.target.checked)} type="checkbox" /> Active</label><fieldset className="field"><legend>Ordered AbilityType affinity</legend>{abilityTypes.map((ability) => <label key={ability}><input checked={affinity.includes(ability)} onChange={() => toggle(ability)} type="checkbox" /> {ability}</label>)}</fieldset><ol>{affinity.map((ability, index) => <li key={ability}>{ability} <button aria-label={`Move ${ability} earlier`} disabled={index === 0} onClick={() => move(index, -1)} type="button">↑</button><button aria-label={`Move ${ability} later`} disabled={index === affinity.length - 1} onClick={() => move(index, 1)} type="button">↓</button></li>)}</ol><button className="button button--gold" disabled={saving || !key || !name || affinity.length === 0} onClick={async () => { setSaving(true); try { const response = await fetch("/api/admin/occupations", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ occupationId: key, name, description: description || null, active, attributeAffinity: affinity }) }); const result = await response.json() as { error?: string }; setMessage(response.ok ? "Occupation saved." : result.error ?? "Occupation could not be saved."); if (response.ok) { setEditing(true); await client.invalidateQueries({ queryKey: ["admin", "occupations"] }); } } finally { setSaving(false); } }}>{saving ? "Saving…" : "Save Occupation"}</button>{message && <p role="status" className="notice">{message}</p>}</section></div>;
}
...
function PlannerWorldGrid({ companions, occupations, propertiesOnRows, validationIssues, visibleCompanions, visibleProperties, world }: { companions: PlannerCompanion[]; occupations: OccupationRecord[]; propertiesOnRows: boolean; validationIssues: PlannerValidationIssue[]; visibleCompanions: string[]; visibleProperties: Array<typeof plannerProperties[number]>; world: typeof plannerWorlds[number] }) {
  type PlannerGridRow = Record<string, string> & { rowLabel: string };
  const headings = propertiesOnRows ? visibleCompanions : visibleProperties;
  const rowHeadings = propertiesOnRows ? visibleProperties : visibleCompanions;
  const rows: PlannerGridRow[] = rowHeadings.map((rowHeading) => ({
    rowLabel: rowHeading,
    ...Object.fromEntries(headings.map((heading) => {
      const companionKey = String(propertiesOnRows ? heading : rowHeading);
      const property = (propertiesOnRows ? rowHeading : heading) as typeof plannerProperties[number];
      const companion = companions.find((entry) => entry.companionKey === companionKey);
      return [String(heading), companion ? plannerValue(companion, world, property, occupations) : "Missing companion"];
    })),
  }));
  const columns: DataTableColumnDef<PlannerGridRow>[] = [
    { accessorKey: "rowLabel", header: propertiesOnRows ? "Property" : "Companion" },
    ...headings.map((heading) => ({
      accessorKey: String(heading),
      cell: ({ row }: { row: { original: PlannerGridRow } }) => { const companionKey = String(propertiesOnRows ? heading : row.original.rowLabel); const property = (propertiesOnRows ? row.original.rowLabel : heading) as typeof plannerProperties[number]; const issueId = cellIssueId(world, companionKey, property); return <span className={validationIssues.some((issue) => issue.cell === issueId) ? "planner-cell-error" : undefined} data-cell-id={issueId}>{row.original[String(heading)]}</span>; },
      header: String(heading),
    })),
  ];
  return <section className={`planner-matrix-group planner-world-${world.toLowerCase()}`}><h3>{world}</h3><DataTable columns={columns} data={rows} getRowId={(row) => row.rowLabel} preferenceKey={`admin.companion-planner.${world}.${propertiesOnRows ? "properties" : "companions"}`} /></section>;
}
...
export function TransformationAuthoringPage() {
  ...
  return <section className="card"><h2>Transformation authoring</h2><p>The trigger is the end of the first Book assigned to the companion. Completion grants the selected Layette and the irreversible Transformation capability; Awareness remains unavailable until that event.</p>{query.isPending ? <p>Loading…</p> : query.isError ? <p className="notice notice--bad">{query.error.message}</p> : query.data && <div className="form-grid"><label className="field">Companion<select className="select" value={companionKey} onChange={(event) => setCompanionKey(event.target.value)}>{query.data.companions.map((entry) => <option key={entry.companionKey} value={entry.companionKey}>{entry.soul?.name ?? `Companion ${entry.companionKey}`} · {entry.companionKey}</option>)}</select></label><label className="field">Layette<select className="select" value={layetteId} onChange={(event) => setLayetteId(event.target.value)}><option value="">Select Layette</option>{query.data.layettes.map((entry) => <option key={entry.layetteId} value={entry.layetteId}>{entry.name} · {entry.layetteId}</option>)}</select></label><button className="button button--gold span-2" disabled={saving || !layetteId} onClick={async () => { setSaving(true); try { const response = await fetch("/api/admin/companion-planner", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "transformation", companionKey, layetteId }) }); const result = await response.json() as { error?: string }; setMessage(response.ok ? "Transformation binding saved." : result.error ?? "Binding could not be saved."); if (response.ok) await query.refetch(); } finally { setSaving(false); } }}>{saving ? "Saving…" : "Save Transformation Binding"}</button></div>}{message && <p className="notice" role="status">{message}</p>}</section>;
}

interface MoneyTransactionView extends Record<string, unknown> { moneyTransactionId: string; occurredAtGameMinute: string; delta: number; withdrawalAmount: number | null; worldInstance: { worldKey: string }; party: { user: { email: string; name: string } } }

export function MoneyAuditPage() {
  const query = useQuery({ queryKey: ["admin", "money"], queryFn: async () => { const response = await fetch("/api/admin/money"); const result = await response.json() as { transactions?: MoneyTransactionView[]; error?: string }; if (!response.ok || !result.transactions) throw new Error(result.error ?? "Money audit could not be loaded."); return result.transactions; } });
  const columns: DataTableColumnDef<MoneyTransactionView>[] = [
    { accessorFn: (row) => row.party.user.name, header: "Account", id: "accountName" },
    { accessorFn: (row) => row.party.user.email, header: "Account email", id: "accountEmail" },
    { accessorFn: (row) => JSON.stringify(row.party), header: "Party", id: "party" },
    { accessorKey: "moneyTransactionId", header: "Transaction ID" },
    { accessorFn: (row) => row.worldInstance.worldKey, header: "World", id: "worldKey" },
    { accessorFn: (row) => JSON.stringify(row.worldInstance), header: "World instance", id: "worldInstance" },
    ...
    { accessorFn: (row) => String(row.recordedAt ?? "—"), header: "Recorded", id: "recordedAt" },
    { accessorFn: (row) => JSON.stringify(row.context ?? null), header: "Context", id: "context" },
  ];
  return <section className="card"><h2>World Transaction Ledger</h2>{query.isPending ? <p>Loading…</p> : query.isError ? <p className="notice notice--bad">{query.error.message}</p> : <DataTable columns={columns} data={query.data} getRowId={(row) => row.moneyTransactionId} preferenceKey="admin.money.transactions" />}</section>;
}
