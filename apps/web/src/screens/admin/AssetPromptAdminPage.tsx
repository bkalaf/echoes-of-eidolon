...
const assetColumns: DataTableColumnDef<AssetRow>[] = [
  ...
  { id: "purposeLinks", header: "Purposes", accessorFn: (asset) => JSON.stringify(asset.purposeLinks) },
  { accessorKey: "objectKey", header: "Final-byte object key" },
  { accessorKey: "sha256", header: "SHA-256" },
  { accessorFn: (asset) => JSON.stringify(asset.technicalMetadata), header: "Technical metadata", id: "technicalMetadata" },
];
...
const promptColumns: DataTableColumnDef<PromptRow>[] = [
  ...
  { accessorFn: (prompt) => JSON.stringify(prompt.versions), header: "All versions", id: "versions" },
];
...
function PromptManager({ outstandingOnly }: { outstandingOnly: boolean }) {
  ...
  const resultMediaKind = selected?.family === "IMAGE" ? "IMAGE" : selected?.family === "MUSIC" ? "AUDIO" : undefined;
  const managedAssets = useQuery({
    enabled: Boolean(resultMediaKind),
    queryKey: ["admin", "assets", "prompt-result", resultMediaKind],
    queryFn: async () => readJson<{ assets: AssetRow[]; total: number }>(await fetch(`/api/admin/assets/?mediaKind=${resultMediaKind}`)),
    retry: false,
  });
