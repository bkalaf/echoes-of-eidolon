...
const settlementColumns: DataTableColumnDef<SettlementWorldView>[] = [
  { accessorFn: (world) => world.settlement.name ?? "Naming pending", header: "Settlement", id: "settlementName" },
  ...
  { accessorFn: (world) => world.settlement.site.regionId, header: "Region", id: "regionId" },
  { accessorFn: (world) => world.settlement.site.longitude, header: "Longitude", id: "longitude" },
  { accessorFn: (world) => world.settlement.site.latitude, header: "Latitude", id: "latitude" },
  { accessorKey: "settlementWorldId", header: "Settlement World ID" },
  { accessorKey: "worldKey", header: "World" },
  { accessorKey: "totalPopulation", header: "Population", cell: ({ row }) => row.original.totalPopulation.toLocaleString() },
  { accessorKey: "latestYear", header: "Latest year" },
  { accessorFn: (world) => world.dominantBreed?.name ?? "None", header: "Dominant Breed", id: "dominantBreedName" },
  { accessorFn: (world) => world.dominantBreed?.breedId ?? "None", header: "Dominant Breed ID", id: "dominantBreedId" },
  { accessorFn: (world) => world.culture?.name ?? "None", header: "Culture", id: "cultureName" },
  { accessorFn: (world) => world.culture?.cultureId ?? "None", header: "Culture ID", id: "cultureId" },
  { accessorFn: (world) => world.populations.map(({ breedId, name, population }) => `${name} · ${breedId}: ${population}`).join(" · "), header: "Breed populations", id: "populations" },
  { cell: ({ row }) => <a className="button button--small" href={`/admin/atlas/settlements/${encodeURIComponent(row.original.settlement.settlementId)}/migrate`}>Migrate</a>, enableColumnFilter: false, enableSorting: false, header: "Actions", id: "actions" },
];
