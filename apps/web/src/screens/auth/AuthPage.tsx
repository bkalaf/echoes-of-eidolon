...
type AuthFields = {
  ...
  username: string;
  code: string;
  eligibilityStatus: "AGE_18_OR_OLDER" | "AGE_14_TO_17_WITH_GUARDIAN_PERMISSION";
};
...
export function AuthPage({ screen }: { screen: PageManifestEntry }) {
  ...
  return <AuthShell><main className="auth-page"><section className="auth-card"><p className="kicker">Account access</p><h1>{copy.title}</h1><p>{copy.description}</p>{screen.screenId === "AUT008" ? <a className="button button--gold" href="/auth/sign-in">Sign in again</a> : screen.screenId === "AUTH02" ? <div className="action-row"><a className="button" href="/">Cancel</a><button className="button button--gold" disabled={busy} onClick={signOut}>Sign Out</button></div> : screen.screenId === "AUTH09" ? <button className="button button--gold auth-submit" disabled={busy} onClick={signInWithPasskey}>{copy.submit}</button> : <form onSubmit={handleSubmit(submit)}>{usesUsername && <label className="field">Username<input className="input" autoComplete="username" {...register("username", { required: true, minLength: 3, maxLength: 30 })} /></label>}{usesEmail && <label className="field">Email<input className="input" type="email" autoComplete="email" {...register("email", { required: true })} /></label>}{usesPassword && <label className="field">Password<input className="input" type="password" autoComplete={["AUTH03", "AUTH05"].includes(screen.screenId) ? "new-password" : "current-password"} {...register("password", { required: true, minLength: 8 })} /></label>}{screen.screenId === "AUTH03" && <fieldset className="field"><legend>Eligibility</legend><label><input type="radio" value="AGE_18_OR_OLDER" {...register("eligibilityStatus", { required: true })} /> 18 or older</label><label><input type="radio" value="AGE_14_TO_17_WITH_GUARDIAN_PERMISSION" {...register("eligibilityStatus", { required: true })} /> 14–17 with guardian permission</label><p className="muted">No date of birth or numeric age requested.</p></fieldset>}{usesCode && <label className="field">{screen.screenId === "AUTH07" ? "Invitation code" : screen.screenId === "AUTH08" ? "6-digit code" : "Reset code"}<input className="input" inputMode="numeric" {...register("code", { required: true, ...(screen.screenId === "AUTH08" ? { minLength: 6, maxLength: 6 } : {}) })} /></label>}{screen.screenId === "AUTH01" && <div className="auth-links"><a href="/auth/forgot-password">Forgot password?</a><a href="/auth/passkeys">Use a passkey</a></div>}{unsupported && <p className="notice notice--warn">This operation requires its owner-supplied issuance and verification contract.</p>}<button className="button button--gold auth-submit" disabled={!isValid || busy || unsupported}>{busy ? "Working…" : copy.submit}</button></form>}{error && <p className="notice notice--bad" role="alert">{error}</p>}{message && <p className="notice notice--good" role="status">{message}</p>}</section>{screen.screenId === "AUTH03" && <aside className="auth-aside"><h2>Privacy-minimal signup</h2><p>Only eligibility status is stored. A 14–17 account must have valid guardian-consent evidence before participation is enabled.</p><p className="notice">Under 14 is not eligible.</p></aside>}</main>{screen.screenId === "AUTH06" && <VerifyEmailModal />}</AuthShell>;
}
