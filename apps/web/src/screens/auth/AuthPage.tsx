...
const authCopy: Record<string, { title: string; description: string; submit: string }> = {
  ...
  AUTH06: { title: "Sign Up", description: "Create your Echoes of Eidolon account.", submit: "Create Account" },
...
export function AuthPage({ screen }: { screen: PageManifestEntry }) {
  ...
  return <AuthShell><main className="auth-page"><section className="auth-card"><p className="kicker">Account access</p><h1>{copy.title}</h1><p>{copy.description}</p>{!supported ? <p className="notice notice--warn">No authentication operation is available for an unregistered screen.</p> : screen.screenId === "AUT008" ? <a className="button button--gold" href="/auth/sign-in">Sign in again</a> : screen.screenId === "AUTH02" ? <div className="action-row"><a className="button" href="/">Cancel</a><button className="button button--gold" disabled={busy} onClick={signOut}>Sign Out</button></div> : screen.screenId === "AUTH09" ? <button className="button button--gold auth-submit" disabled={busy} onClick={signInWithPasskey}>{copy.submit}</button> : <form onSubmit={handleSubmit(submit)}>{usesUsername && <label className="field">Username<input className="input" autoComplete="username" {...register("username", { required: true })} /></label>}{usesEmail && <label className="field">Email<input className="input" type="email" autoComplete="email" {...register("email", { required: true })} /></label>}{usesPassword && <label className="field">Password<input className="input" type="password" autoComplete={["AUTH03", "AUTH05"].includes(screen.screenId) ? "new-password" : "current-password"} {...register("password", { required: true })} /></label>}{screen.screenId === "AUTH03" && <fieldset className="field"><legend>Age eligibility</legend><label><input type="radio" value="ADULT_18_PLUS" {...register("eligibilityStatus", { required: true })} /> 18 or older</label><label><input type="radio" value="MINOR_14_17_GUARDIAN_CONSENTED" {...register("eligibilityStatus", { required: true })} /> 14–17; guardian consent evidence required before participation</label><p className="muted">No date of birth or exact age is collected. A minor may create an account, but cannot participate until an active guardian-consent record from an approved verification method exists.</p></fieldset>}{screen.screenId === "AUTH08" && <button className="button" type="button" disabled={busy} onClick={sendTwoFactorCode}>Send code</button>}{usesCode && <label className="field">{screen.screenId === "AUTH07" ? "Invitation code" : screen.screenId === "AUTH08" ? "6-digit code" : "Reset code"}{screen.screenId === "AUTH07" ? <input className="input" {...register("code", { required: true })} /> : <OtpInput {...register("code", { required: true, minLength: 6, maxLength: 6 })} />}</label>}{screen.screenId === "AUTH01" && <div className="auth-links"><a href="/auth/forgot-password">Forgot password?</a><a href="/auth/passkeys">Use a passkey</a></div>}<button className="button button--gold auth-submit" disabled={!isValid || busy || unsupported}>{busy ? "Working…" : copy.submit}</button></form>}{error && <p className="notice notice--bad" role="alert">{error}</p>}{message && <p className="notice notice--good" role="status">{message}</p>}</section>{screen.screenId === "AUTH03" && <aside className="auth-aside"><h2>Privacy-minimal signup</h2><p>Adults store only ADULT_18_PLUS. The application does not collect date of birth or exact age.</p><p className="notice">Under 14 is ineligible.</p></aside>}</main>{screen.screenId === "AUTH06" && <VerifyEmailModal />}</AuthShell>;
}
