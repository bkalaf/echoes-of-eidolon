...
function FeaturesPage() {
  return <><div className="action-row"><a className="button" href="/">Back</a><a className="button" href="/">Home</a></div><PageHead eyebrow="Features" title="Nine ways Echoes plays differently." description="Explore the systems behind the world, conversation, challenges and story." /><section className="features-overview"><div className="video-panel video-panel--features"><video controls preload="metadata" src={managedAssetUrl("video.power-of-three")}>Your browser does not support the captioned feature video.</video><div><strong>What makes Echoes of Eidolon different?</strong><small>Captioned feature video</small></div></div><div className="feature-grid">{publicFeatures.map((feature) => <a className="feature-tile" href={`/features/${feature.slug}`} key={feature.slug}><img src={feature.image ?? feature.icon} alt="" /><h2>{feature.title}</h2><p>{feature.summary}</p><span>View feature →</span></a>)}</div></section></>;
}
