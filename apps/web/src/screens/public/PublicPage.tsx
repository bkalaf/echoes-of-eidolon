...
function ContactPage() {
  ...
  const targets = [["General company inquiry", "5 business days"], ["Press and media", "2 business days"], ["Business partnerships and licensing", "7 business days"], ["Accessibility feedback", "3 business days"], ["Privacy and data-rights inquiry", "3 business days acknowledgement; applicable legal deadlines still control"], ["Legal notice", "3 business days acknowledgement"], ["Security report", "2 business days acknowledgement; critical reports prioritized as soon as practicable"], ["Historical/cultural correction or sourcing concern", "5 business days acknowledgement; review status/substantive update targeted within 15 business days"]];
  return <><PageHead eyebrow="Contact" title="Contact Us" description="Choose the company topic that best matches your message." /><p className="notice"><strong>Player support messages should be sent from the Support tab, not this webform.</strong> <a href="/account/support">Go to Support</a></p><div className="contact-layout"><form className="card" onInput={(event) => setComplete(event.currentTarget.checkValidity())} onSubmit={(event) => event.preventDefault()}><h2>Company Contact</h2><div className="topics">{topics.map((item) => <button className={`topic ${item === topic ? "selected" : ""}`} type="button" onClick={() => setTopic(item)} key={item}>{item}</button>)}</div><label className="field">Reply email<input className="input" type="email" placeholder="you@example.com" required /></label><label className="field">Message<textarea className="textarea" placeholder="Tell us what you need help with." required /></label><div className="action-row"><a className="button" href="/">Back</a><a className="button" href="/">Home</a><button className="button button--gold" disabled>Send unavailable</button></div>{!complete && <small>Complete the required fields.</small>}<p className="notice notice--warn">Company contact delivery is owner-deferred until a company-contact recipient and retention/routing contract are supplied. The support recipient is not reused.</p></form><aside className="card"><h2>Initial response targets</h2><table className="simple-table"><tbody>{targets.map(([name, target]) => <tr key={name}><td>{name}</td><td>{target}</td></tr>)}</tbody></table><p>These are acknowledgement targets, not guaranteed resolution times. Business days exclude weekends and company-observed holidays.</p></aside></div></>;
}
...
function DonationPage({ checkout, eligible }: { checkout?: boolean; eligible?: boolean }) {
  return <><PageHead eyebrow="Donation" title={checkout ? "Donation Checkout" : "Donate"} description={checkout ? "Donation payment remains separate from merchandise and shipping." : "Support Echoes of Eidolon without purchasing merchandise."} />{checkout ? <div className="checkout-layout"><section className="card"><h2>Donation amount</h2><p className="price">$50.00</p><p>The reviewed eligible-participant grant is six months of Member time.</p><p>No merchandise order or shipping address is created.</p></section><aside className="card"><h2>Summary</h2><p>Donation <strong>$50.00</strong></p><p>Member time <strong>+6 months</strong></p><button className="button button--gold" disabled>Donate unavailable</button><p className="notice notice--warn">Stripe submission is blocked until participant eligibility, grant persistence, webhook idempotency, and receipt ownership are supplied.</p></aside></div> : <div className="grid-2"><article className="card"><h2>{eligible ? "Eligible participant" : "Guest information"}</h2><p>{eligible ? "This reviewed state is eligible for donation-linked Member time, but live eligibility is not yet owned by the account model." : "Sign in to see account-specific eligibility before continuing."}</p><a className="button button--gold" href={eligible ? "/donate/checkout" : "/auth/sign-in"}>{eligible ? "Review donation" : "Sign in"}</a></article><article className="card"><h2>Separate from merchandise</h2><p>A donation does not create a merchandise order or shipping address.</p></article></div>}</>;
}
