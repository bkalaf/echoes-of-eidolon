...
function world(totalPopulation = 8) {
  return {
    ...
    populationEvents: [
      { breed: { name: "Origin Breed" }, breedId: "BREED-1", populationDelta: 10, year: 1 },
      { breed: { name: "Origin Breed" }, breedId: "BREED-1", populationDelta: -2, year: 2 },
    ],
...
describe("Settlement list projection", () => {
  it("derives current Breed counts and latest year from the append-only population ledger", async () => {
    ...
    expect(result[0]).toMatchObject({ latestYear: 2, populations: [{ breedId: "BREED-1", name: "Origin Breed", population: 8 }], totalPopulation: 8 });
