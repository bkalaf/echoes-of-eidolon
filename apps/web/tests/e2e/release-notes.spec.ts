...
test("RN-026 RN-030 a draft release detail fails closed as 404", async ({ request }) => {
  const response = await request.get("/status/releases/0.2.0");
  expect(response.status()).toBe(404);
  expect(await response.text()).not.toContain("A verified release summary");
});

test("RN-032 RN-033 only published release details are crawlable", async ({ request }) => {
  const sitemap = await request.get("/sitemap.xml");
  expect(await sitemap.text()).not.toContain("/status/releases/0.2.0</loc>");
});
