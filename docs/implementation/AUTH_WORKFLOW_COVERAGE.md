...
| AUTH05 Reset password | `/auth/reset-password`; preserved email optional | static account identity or email fallback, six-position `OtpInput`, two canonical Password controls; reset OTP | dedicated Password Reset state and Sign In action | malformed/mismatched disabled; wrong/expired/used/rate-limit error | yes | yes | yes: type/paste/backspace/replace, resend, reset, old fails/new succeeds, refresh fallback | controlled credential; production structural check without rotating an owner unintentionally |
...
| ACC024 Change password | `/account/profile?state=ACC024`; authenticated credential account | current password, new password, confirmation; Better Auth change-password | dedicated Password Changed state; current session preserved | wrong current password fails; mismatch/empty values cannot submit | yes | Better Auth route | yes: wrong current, independent visibility, success, old fails/new succeeds | controlled disposable account only |
...
| Password change while signed in | ACC024 modal from Account Profile | current/new/confirmation, native Better Auth verification, success, old fails/new succeeds |
